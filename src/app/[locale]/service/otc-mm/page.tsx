'use client';

import React from 'react';
import BreadcrumbItem from '@/components/Breadcrumb/BreadcrumbItem';
import CtaOne from '@/components/Section/CTA/CtaOne';
import Link from 'next/link';
import Image from 'next/image';
import { useTranslation } from '@/i18n/TranslationContext';

export default function LiquiditySolutionsPage() {
  const { t, locale } = useTranslation();

  // Mock data for liquidity services
  const liquidityServices = [
    {
      id: 1,
      title: 'OTC Desk',
      description:
        'Facilitating large token and NFT trades with minimal market impact and maximum confidentiality.',
      icon: 'M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z',
    },
    {
      id: 2,
      title: 'Algorithmic Market-Making',
      description:
        'Ensuring deep liquidity and tight spreads for tokens and NFTs across multiple platforms.',
      icon: 'M19.428 15.428a2 2 0 00-1.022-.547l-2.387-.477a6 6 0 00-3.86.517l-.318.158a6 6 0 01-3.86.517L6.05 15.21a2 2 0 00-1.806.547M8 4h8l-1 1v5.172a2 2 0 00.586 1.414l5 5c1.26 1.26.367 3.414-1.415 3.414H4.828c-1.782 0-2.674-2.154-1.414-3.414l5-5A2 2 0 009 10.172V5L8 4z',
    },
    {
      id: 3,
      title: 'Custom Liquidity Solutions',
      description:
        'Tailored strategies to meet the unique liquidity needs of your token or NFT project.',
      icon: 'M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8c1.11 0 2.08.402 2.599 1M12 8V7m0 1v8m0 0v1m0-1c-1.11 0-2.08-.402-2.599-1M21 12a9 9 0 11-18 0 9 9 0 0118 0z',
    },
  ];

  // Mock data for liquidity metrics
  const liquidityMetrics = [
    {
      metric: '$1B+',
      description: 'Total Liquidity Provided',
    },
    {
      metric: '500+',
      description: 'Tokens and NFTs Supported',
    },
    {
      metric: '24/7',
      description: 'Global Trading Availability',
    },
    {
      metric: '99.9%',
      description: 'Uptime for Market Operations',
    },
  ];

  return (
    <>
      <BreadcrumbItem
        link="Liquidity Solutions"
        img="/images/banner/about1.png"
        title="Liquidity Solutions for Tokens and NFTs"
        desc="Our OTC desk and algorithmic market-making ensure smooth trading with minimal slippage."
      />

      <div className="container mx-auto px-4 py-16">
        <div className="max-w-4xl mx-auto">
          <div className="mb-16">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-10 items-center">
              <div>
                <h2 className="text-3xl font-bold mb-6">
                  Seamless Liquidity for Digital Assets
                </h2>
                <p className="mb-6">
                  Golden Goose Ventures offers comprehensive incubation and
                  advisory services designed specifically for blockchain
                  startups and projects. Our team of industry experts provides
                  hands-on support across every aspect of building a successful
                  blockchain business, from technical architecture to
                  tokenomics, regulatory compliance, and go-to-market strategy.
                </p>
                <p>
                  Whether you are a project owner, institutional investor, or
                  trader, our tailored liquidity solutions help you navigate the
                  complexities of digital asset markets with confidence.
                </p>
              </div>
              <div>
                <Image
                  src="/images/blog/930x593.png"
                  alt="Liquidity Solutions"
                  className="rounded-lg shadow-lg w-full"
                  width={930}
                  height={593}
                />
              </div>
            </div>
          </div>

          <div className="mb-16">
            <h2 className="text-2xl font-bold mb-8 text-center">
              Our Liquidity Services
            </h2>

            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-10">
              {liquidityServices.map((service) => (
                <div
                  key={service.id}
                  className="bg-white p-6 rounded-lg shadow-md"
                >
                  <div className="w-12 h-12 bg-brown-100 rounded-full flex items-center justify-center mb-4">
                    <svg
                      xmlns="http://www.w3.org/2000/svg"
                      className="h-6 w-6 text-brown-600"
                      fill="none"
                      viewBox="0 0 24 24"
                      stroke="currentColor"
                    >
                      <path
                        strokeLinecap="round"
                        strokeLinejoin="round"
                        strokeWidth={2}
                        d={service.icon}
                      />
                    </svg>
                  </div>
                  <h3 className="text-lg font-semibold mb-2">
                    {service.title}
                  </h3>
                  <p className="text-sm">{service.description}</p>
                </div>
              ))}
            </div>

            <div className="mb-10">
              <div className="grid grid-cols-2 md:grid-cols-4 gap-6 text-center">
                {liquidityMetrics.map((item, index) => (
                  <div
                    key={index}
                    className="bg-white p-5 rounded-lg shadow-md"
                  >
                    <div className="text-3xl font-bold text-brown-500 mb-2">
                      {item.metric}
                    </div>
                    <p className="text-sm">{item.description}</p>
                  </div>
                ))}
              </div>
            </div>

            <div className="text-center">
              <Link
                href="/service/liquidity-solutions/apply"
                className="inline-block px-6 py-3 bg-brown-500 text-white rounded-md font-medium hover:bg-brown-600 transition-colors"
              >
                Apply for Liquidity Services
              </Link>
            </div>
          </div>

          <div className="bg-gray-50 p-8 rounded-lg text-center mb-12">
            <h2 className="text-2xl font-bold mb-4">
              Ready to Optimize Your Liquidity?
            </h2>
            <p className="mb-6">
              Whether you need OTC trading or algorithmic market-making, our
              team is ready to provide tailored solutions for your digital
              assets.
            </p>

            <div className="flex flex-col sm:flex-row gap-4 justify-center">
              <Link
                href={`/${locale}/contact`}
                className="inline-block px-6 py-3 bg-tan-dark text-white rounded-md font-medium hover:bg-brown transition-colors"
              >
                Contact Us
              </Link>

              <Link
                href={`/${locale}/contact`}
                className="inline-block px-6 py-3 border border-brown-500 text-brown-500 rounded-md font-medium hover:bg-brown hover:text-white transition-colors"
              >
                Schedule a Consultation
              </Link>
            </div>
          </div>

          <CtaOne />
        </div>
      </div>
    </>
  );
}
